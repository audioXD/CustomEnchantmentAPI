...
public abstract class RequirementAbstract implements Requirement, Serializable {
	...
	public static boolean isRequirementsMet(Iterable<Requirement> requirements, CommandSender sender, CEAPICommand command, boolean verboose) {
		String error = getRequirementsError(requirements, sender, command, verboose);
		if(error != null && verboose) sender.sendMessage(error);
		return error == null;
	}

	public static String getRequirementsError(Iterable<Requirement> requirements, CommandSender sender, CEAPICommand command, boolean verboose) {
