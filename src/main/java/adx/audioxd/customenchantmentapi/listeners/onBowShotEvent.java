...
public class onBowShotEvent extends CEPLListener {
	...
	public void bowShoot(EntityShootBowEvent event) {
		for (Enchanted ench : this.getEnchantments(event.getBow())) {

			event.getProjectile().setMetadata(salt + ench.getEncnantment().getName(),
					new FixedMetadataValue(this.plugin, ench.getLvl()));

			EBowShootEvent e = new EBowShootEvent(ench.getLvl(), event.getBow(), event.getEntity(), event.getProjectile());
	...
	@EventHandler(priority = EventPriority.LOWEST, ignoreCancelled = true)
	public void hitedByArrow(EntityDamageByEntityEvent event) {
		...
		for (Enchantment ench : EnchantmentRegistery.geEnchantmentsArray()) {
			if (arrow.hasMetadata(salt + ench.getName())) {
				...
				EArrowHitEvent e = new EArrowHitEvent(targer, lvl, arrow, event.getDamage());
				e.setCancelled(event.isCancelled());
				{
					ench.fireEvent(e);
				}
	...
	public void arrowLandEvent(ProjectileHitEvent event) {
		...
		for (Enchantment ench : EnchantmentRegistery.geEnchantmentsArray()) {
			if (arrow.hasMetadata(salt + ench.getName())) {
				...
				EArrowLandEvent e = new EArrowLandEvent(arrow, lvl);
				ench.fireEvent(e);
			}
		}
	}
